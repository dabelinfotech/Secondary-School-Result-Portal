...
let cachedClasses  = [];
let cachedMe       = null;
let allSubjects    = [];   // full subject list for assignment checkboxes
let editingAdminId = null; // tracks which staff is being edited in assignment modal
...
function applyRoleUI() {
  ...
  // Hide nav items staff should not access
  ...
  // Hide quick-action buttons on overview that staff can't use
...
async function loadSubjectsForClass(cls) {
  ...
  const hint   = document.getElementById('result-subject-hint');
  const isJSS  = cls && /^jss/i.test(cls.trim());

  try {
    const res  = await fetch('/api/admin/subjects?class=' + encodeURIComponent(cls || ''));
    const data = await res.json();
    // Store all subjects for assignment checkboxes (use first successful fetch)
    if (data.all && !allSubjects.length) allSubjects = data.all;
    let subjects = data.subjects || [];

    // Staff: restrict to their assigned subjects only
    ...
    if (hint) hint.textContent = isJSS ? '(Junior Secondary School)' : '(Senior Secondary School)';
  } catch (_) {
...
function populateSubjectCheckboxes(containerId, selectedSubjects) {
  ...
  if (!container) return;
  const list = allSubjects.length ? allSubjects : (window._fallbackSubjects || []);
...
function subjectAbbr(name) {
  const map = {
    ...
    'National Values and Education':  'NVE',
    'Pre Vocational Studies':         'PVS',
    'Cultural and Creative Arts':     'CCA',
    'History':                        'Hist',
    'Business Studies':               'B.Std',
    'French':                         'French',
    'Coding':                         'Coding',
  };
