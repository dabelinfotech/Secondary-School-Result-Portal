...
const SSS_SUBJECTS = [
  ...
  'Technical Drawing', 'Geography', 'Yoruba Language', 'Igbo Language',
  'Literature in English', 'Government', 'Food and Nutrition', 'Marketing',
  'Information Technology'
];

const JSS_SUBJECTS = [
  'Mathematics', 'English Language', 'Basic Science and Technology',
  'National Values and Education', 'Pre Vocational Studies',
  'Cultural and Creative Arts', 'History', 'Yoruba Language', 'Igbo Language',
  'Business Studies', 'French', 'Coding'
];

function getSubjectsForClass(cls) {
  if (cls && /^jss/i.test(String(cls).trim())) return JSS_SUBJECTS;
  return SSS_SUBJECTS;
}
...
router.get('/subjects', requireAuth, (req, res) => {
  ...
  const all = [...new Set([...SSS_SUBJECTS, ...JSS_SUBJECTS])];
  res.json({ success: true, subjects, ss: SSS_SUBJECTS, jss: JSS_SUBJECTS, all });
});
